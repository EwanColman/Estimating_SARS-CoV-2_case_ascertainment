...
def f(theta):
    ...
    Z=sum([abs(population_of[region]*rate[interval][t]/100 - 100*sum([theta_times_I[ONS_day[t]-j]*S_pcr[j]/daily_theta[ONS_day[t]-j] for j in range(testable_period)])) for t in range(len(rate[interval]))])   
...
#    m=1+int(n/3)
    ...
    plt.plot(ONS_day,[100*100*sum([theta_times_I[t-j]*S_pcr[j]/(daily_theta[t-j]*population_of[region]) for j in range(testable_period)]) for t in ONS_day])
    ...
    for interval in reporting_multiplier:
        ...
        I[interval]=I[interval]+[100*100*theta_times_I[t]/(daily_theta[t]*population_of[region]) for t in range(ONS_day[0],len(theta_times_I))]
