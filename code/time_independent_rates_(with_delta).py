...
x1_values=[100/i for i in range(5,100)]
x2_values=[100/i for i in range(5,100)]
x3_values=[100/i for i in range(10,55)]
...
output={'Group':[],'Wild_Rate':[],'Wild_Lower':[],'Wild_Upper':[],
        'Alpha_Rate':[],'Alpha_Lower':[],'Alpha_Upper':[],
        'Delta_Rate':[],'Delta_Lower':[],'Delta_Upper':[]}
...
for age in population_of:
    ...
    df=df[(df['days_since_march1']>Sep1)] 
    ...
    # we use the loop below to find the index of when the change occurs
    delta_index=0
    ...
    for interval in ['Rate','Lower','Upper']:
        ...
        output['Delta_'+interval].append(round(100/x3))
...
for region in population_of:
    ...
    for t in ONS_day:
        ...
        # the date we switch from wild to delta 
        if t<Mar1:
            # this will be the latest t which is before March 1st 2021
            delta_index=delta_index+1
...
pd.DataFrame(output).to_csv('../output/time_independent_rates_(with_delta).csv',index=False)
